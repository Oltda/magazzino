...
class PdfSheet:
    ...
    def to_pdf(self):
        ...
        stock = StockItems.query.filter(StockItems.user_id == self.user_id).order_by(StockItems.product_code.asc())
        ...
        response.headers.set('Content-Disposition', 'attachment', filename='stock_report.pdf')
        response.headers.set('Content-Type', 'application/pdf')

        return response

    def filter_code_pdf(self, code):
        ...
        stock = StockItems.query.filter(StockItems.user_id == self.user_id).filter(StockItems.product_code == code).order_by(StockItems.id.asc())
